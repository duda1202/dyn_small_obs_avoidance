...
void ImuProcess::Reset() 
{
  ...
  ROS_WARN("Finished reset ImuProcess, first frame? %d", b_first_frame_);
}
...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  ROS_INFO("IMU Initializing: %.1f %%. first frame? %d", double(N) / MAX_INI_COUNT * 100, b_first_frame_);
  Eigen::Vector3d cur_acc, cur_gyr;
  
  if (b_first_frame_)
  {
    ROS_INFO("IMU first frame, calling reset.");
    Reset();
    N = 1;
  ...
  ROS_WARN("Finished init ImuProcess, first frame? %d", b_first_frame_);
}
